...
1. **ネタ取得**: Notion MCP で「ブログネタ」ページ(下記の固定URL)を取得し、`## 未処理` セクションの**最上行(箇条書き 1 件)**を処理対象として抜き出す。
   - ページURL: `https://app.notion.com/p/371d745d3f8380e49431e784cd244d77`
   - **必ずこの URL(ページID)で取得すること**。タイトルでの検索はしない(ページタイトルは変更されることがある)。
