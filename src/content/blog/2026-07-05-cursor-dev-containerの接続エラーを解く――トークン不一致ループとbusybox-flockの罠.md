...
切り分けの実用的な目印として、`docker exec` では入れるのに Dev Container では入れない、という違いが効く。これはコンテナの起動や OS 要件ではなく、ホスト Cursor ↔ cursor-server の層に問題があることを示す。なお本記事で罠を紹介した順序(遭遇順＝上位層から)と、ここで勧める切り分け順(下位層から)はあえて逆にしてある。
